...
def convert_to_coco_api(ds):
    ...
    categories = set()
    for imgs, trgts in ds:
        # find better way to get target
        # targets = ds.get_annotations(img_idx)
        for img, targets in zip(imgs, trgts):
            ...
            areas = targets["area"]
            iscrowd = [False] * len(targets["iscrowd"])
            if "masks" in targets:
                ...
                rles = [
                    mask_util.encode(np.array(mask[:, :, np.newaxis], dtype=np.uint8, order="F"))[0] for mask in
                    masks
                ]
                for rle in rles:
